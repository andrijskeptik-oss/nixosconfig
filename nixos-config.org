...
#+end_src
** оборудование загрузчик и ядро
#+header: :noweb-ref boot-vinga  
#+begin_src nix
...
#+BEGIN_SRC nix :noweb-ref filesystem-vinga
  fileSystems."/" =
    { device = system_disk;
      fsType = "btrfs";
      options = [ 
...
** services 
*** +bluetooth
#+header: :noweb-ref services-vinga
#+begin_src nix
services.blueman.enable = true;
#+end_src
*** +sound
#+begin_src nix :noweb-ref services-vinga
...
#  };
#+end_src
Обязaтельно запустить в bash
#+begin_src bash
  sudo smbpasswd -a andrey
#+end_src
*** +net
#+begin_src :noweb-ref services-vinga
...
** packages
*** Сеть
#+begin_src nix :noweb-ref packages-vinga
    #Утилиты
...
**** cortile
пакет
#+header: :tangle ~/Linux/nixosconfig/pkgs/cortile/default.nix :noweb yes :mkdirp yes
#+begin_src nix
...
** enviroments
variables
#+begin_src nix :noweb-ref enviroments-vinga
...
** пакеты
#+header: :noweb-ref packages-vinga
#+begin_src nix
...
#+end_src
** нужно задать кеш для скачивания пакетов
посмотреть, можна ли задать в пакетах системы
#+begin_src bash
  nix-env -iA nixpkgs.cachix
  cachix use cuda-maintainers
#+end_src
** среда разработки Python
#+header: :tangle ~/AI_work/shell.nix :noweb yes :mkdirp yes
#+begin_src nix
...
pkgs.mkShell {
  buildInputs = with pkgs; [
    python3
    python311Packages.pytorch-bin # Версия с поддержкой CUDA
    python311Packages.transformers
    cudaPackages.cudatoolkit
    linuxPackages.nvidia_x11
    nvtopPackages.nvidia
  ...
  shellHook = ''
    export LD_LIBRARY_PATH=${pkgs.linuxPackages.nvidia_x11}/lib:${pkgs.cudaPackages.cudatoolkit}/lib
    echo "NVIDIA AI Development Environment Ready"
  '';
}
#+end_src
* конфигуация программ системы
** все конфигурации
#+header: :tangle ~/Linux/nixosconfig/programm-configs.nix :noweb yes :mkdirp yes
#+begin_src nix
#{ config, pkgs, ... }:
#{ 
#  imports =
...
#+END_SRC
** clifm
,#+BEGIN_SRC nix :tangle ~/Linux/nixosconfig/as-clifm.nix :noweb yes :mkdirp yes
{pkgs, lib, ...}:
...
#+end_src
*** конфиг
#+header: :tangle ~/.config/hypr/hyprland.conf :mkdirp yes :noweb yes
#+begin_src bash 
# Настройка Master-режима (одно большое слева, остальные стеком справа)
  general {
      layout = master
      gaps_in = 5
      gaps_out = 10
  ...
  master {
      new_status = master
      mfact = 0.55 # Основное окно чуть больше половины
  }

  # Автозапуск панели и трея
  exec-once = nwg-panel 
...
#+end_src
* vinga - компʼютер с внутреннего диска
#+header: :tangle ~/Linux/nixosconfig/configuration.nix :mkdirp yes :noweb yes
#+begin_src nix
{ config, pkgs, lib, ... }:
let as_host="vinga";
...
#+end_src
#+header: :tangle ~/Linux/nixosconfig/flake.nix :mkdirp yes :noweb yes
#+begin_src nix
  <<flake>>
#+end_src
* vinga-usb - компʼютер с usb  диска 
,#+header: :tangle ~/Linux/nixosconfig-test/vinga-nix-usb/configuration.nix :mkdirp yes
#+begin_src nix
{ config, pkgs, ... }:
let as_host="vinga usb";
system_disk="/dev/disk/by-uuid/b6affbcd-6022-4593-a686-3070bdc8bc64";
