...
#+BEGIN_SRC nix :noweb-ref filesystem-vingo
  fileSystems."/" =
    { device = system_disk;
      fsType = "btrfs";
      options = [ 
...
** services 
*** bluetooth
#+header: :noweb-ref services-vingo
#+begin_src nix
services.blueman.enable = true;
#+end_src
*** +sound
#+begin_src nix :noweb-ref services-vingo
...
#+end_src
*** +net
#+begin_src :noweb-ref services-vingo
...
** packages
*** Сеть
#+begin_src nix :noweb-ref packages-vingo
    #Утилиты
...
**** cortile
пакет
#+header: :tangle ~/Linux/nixosconfig/vingo-nix/pkgs/cortile/default.nix :noweb yes :mkdirp yes
#+begin_src nix
...
** enviroments
variables
#+begin_src nix :noweb-ref enviroments-vingo
...
** все конфигурации
#+header: :tangle ~/Linux/nixosconfig/vingo-nix/programm-configs.nix :noweb yes :mkdirp yes
#+begin_src nix
#{ config, pkgs, ... }:
#{ 
#  imports =
...
#+END_SRC
** clifm
,#+BEGIN_SRC nix :tangle ~/Linux/nixosconfig/vingo-nix/as-clifm.nix :noweb yes :mkdirp yes
{pkgs, lib, ...}:
...
* vingo - компʼютер с внутреннего диска
#+header: :tangle ~/Linux/nixosconfig/vingo-nix/configuration.nix :mkdirp yes :noweb yes
#+begin_src nix
{ config, pkgs, lib, ... }:
let as_host="vingo";
...
* vingo-usb - компʼютер с usb  диска 
,#+header: :tangle ~/Linux/nixosconfig-test/vingo-nix-usb/configuration.nix :mkdirp yes
#+begin_src nix
{ config, pkgs, ... }:
let as_host="vingo usb";
